...
import { XbpPairedDeviceStore, type PairedDevice } from "./xbp-paired-device-store.js";
import { XbpAuditSink } from "./xbp-audit-sink.js";
import { XbpPairingHost } from "./xbp-pairing-host.js";
import { createLanWebSocketHost, primaryLanIPv4 } from "./lan-websocket-transport.js";
...
export class XbpHostService {
  ...
  constructor(
    ...
    this.pairedStore = new XbpPairedDeviceStore({ dir: opts.dir, secureStorage: opts.secureStorage });
  }
  ...
  async start(): Promise<{ listening: boolean; addr: string | null; port: number | null }> {
    ...
    this.unsubscribe = this.opts.subscribeChanges(() => this.peerSession?.notifyChanged());
    this.enabled = true;
  ...
  async startPairing(): Promise<PairingOffer> {
    ...
    return this.pairingHost!.createOffer({ url: `ws://${addr}:${this.lan!.port}` });
  }
