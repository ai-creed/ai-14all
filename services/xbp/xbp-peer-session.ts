...
import { Peer, type Identity, type SodiumBackend, type Transport } from "@xavier/xbp/node";
import {
...
export class XbpPeerSession {
  ...
  constructor(
    ...
    this.coalescer = createCoalescer(() => {
      if (this.peer && this.phoneNode) {
        this.peer.emit(this.phoneNode, SESSION_CHANGED_TOPIC, { changedAt: now() });
      }
    }, opts.coalesceMs ?? 250);
  }
  ...
  attach(
    ...
    this.phoneNode = peer.addPeer(phoneSignPub, phoneBoxPub, grantedPermissions);
