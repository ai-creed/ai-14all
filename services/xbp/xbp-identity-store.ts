...
export class XbpSecureStorageUnavailableError extends Error {
	constructor() {
		super(
			"OS secure storage is unavailable; refusing to persist XBP keys in plaintext",
		);
...
export class XbpIdentityStore {
	...
	load(): { identity: Identity; nodeId: string } {
		...
		if (existsSync(this.path)) {
			const plain = this.opts.secureStorage.decryptString(
				readFileSync(this.path),
			);
			identity = deserializeIdentity(JSON.parse(plain));
		} else {
			identity = generateIdentity(this.opts.backend);
			const cipher = this.opts.secureStorage.encryptString(
				JSON.stringify(serializeIdentity(identity)),
			);
