...
import {
	...
	type AgentResumeBridgeReply,
	type AgentResumeBridgeRequest,
...
/**
 * Main-process bridge that forwards agent conversation-resume registrations to
 * the renderer and awaits an ack. Deliberate duplication of
 * {@link AgentAttentionBridge} / {@link SessionNoteBridge} — the three bridges
 * share this ready/goodbye/request/reply + pending-map + timeout shape.
 * Consolidating all three into a generic is out of scope for this plan and left
 * as a follow-up refactor.
 */
export class AgentResumeBridge {
	...
	constructor(
		...
		this.ipcMain.on(AGENT_RESUME_BRIDGE_READY, this.readyHandler);
		this.ipcMain.on(AGENT_RESUME_BRIDGE_GOODBYE, this.goodbyeHandler);
		this.ipcMain.on(AGENT_RESUME_BRIDGE_REPLY, this.replyHandler);
	}

	report(body: Omit<AgentResumeBridgeRequest, "id">): Promise<void> {
		if (this.disposed) {
		...
		const id = randomUUID();
		const req: AgentResumeBridgeRequest = { id, ...body };
		return new Promise<void>((resolve, reject) => {
			...
			wc.send(AGENT_RESUME_BRIDGE_REQUEST, req);
		});
	}
	...
	dispose(): void {
		...
		this.disposed = true;
		this.ipcMain.removeListener(AGENT_RESUME_BRIDGE_READY, this.readyHandler);
		this.ipcMain.removeListener(
			AGENT_RESUME_BRIDGE_GOODBYE,
			this.goodbyeHandler,
		);
		this.ipcMain.removeListener(AGENT_RESUME_BRIDGE_REPLY, this.replyHandler);
		for (const [, p] of this.pending) {
			clearTimeout(p.timer);
