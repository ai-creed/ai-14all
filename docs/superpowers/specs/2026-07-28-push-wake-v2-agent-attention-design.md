...
2. **Suppress-while-connected** — if the host currently holds a live, authenticated phone connection, skip the send and audit `suppressed-connected`. No existing signal expresses this (the SDK peer is connectionless and pairing-ceremony state is reset after confirmation); the child spec §3 defines it as an authenticated capability call bound to the specific socket that carried it — that socket still open AND the call within a 45s recency window — so an unrelated socket can never sustain suppression. Known edge: iOS may keep the socket up ~30s after backgrounding; the recency half bounds that window, and a ping lost to the residue is recovered by the pull on next open. Accepted.
3. **Global coalesce** — if any ping **attempt** (either detector) occurred within the last 60s, skip and audit `coalesced`. Attempt-reservation semantics: the persisted timestamp advances before the send and regardless of the send's outcome — duplicates are the forbidden direction, and a ping suppressed after a failed attempt is recovered by the pull (child §3 has the exact contract). Pings are content-free, so one ping already summons the user to the aggregate state.
