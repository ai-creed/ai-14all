...
- **Create at PTY creation, not at catalog publication.** `TerminalService.createSession()` attaches its `onData` handler before returning (`services/terminals/terminal-service.ts:162-202`), and the renderer can publish a catalog entry only after `terminals.create()` resolves (`src/features/terminals/hooks/use-terminal-session.ts:88-120`) — a mirror created at upsert time has already missed bytes. Therefore **every** `TerminalService` session gets a headless mirror from birth, teed inside `createSession()` alongside the existing output batcher; the catalog upsert merely **binds** agent identity to the already-running mirror. The mirror is constructed with the exact `cols`/`rows` passed to `pty.spawn` (the §1 shared spawn constants) and with **`allowProposedApi: true`** — the headless build's `parser` and `buffer` getters both throw without it (`node_modules/xterm/src/headless/public/Terminal.ts:69-105`, `typings/xterm.d.ts:28-32`), and the serializer depends on both; this is a main-process instance, so the renderer's lazy-`allowProposedApi` startup-race gotcha (`TerminalPane`) does not apply. Pin the exact `@xterm/headless` version — handler ordering and the proposed-API surfaces are version-sensitive. No pre-catalog output or resize can be lost, by construction.
...
- **Epoch manager:** epoch is a **monotonically increasing integer within a serializer run** — `epoch++` and full dirty-reset on: resize/reflow, alt-screen enter/exit, scrollback clear (ED 3, which mutates history and would otherwise break trim accounting), terminal hard reset (RIS, `ESC c` — headless xterm recreates the entire `BufferSet` and discards every line: `node_modules/xterm/src/common/InputHandler.ts:351,3268-3275` → `src/headless/Terminal.ts:49-52` → `src/common/buffer/BufferSet.ts:38-48`; without a bump, stale phone rows would survive under reused line IDs), and mirror (re)creation. **RIS detection (two-phase, normative):** registered ESC handlers run newest-first, and returning `false` is precisely what lets xterm's built-in RIS handler run afterward (`node_modules/xterm/src/common/parser/EscapeSequenceParser.ts:714-726`) — so the observer executes *before* the buffer reset and must not read the buffer. Phase 1: the `parser.registerEscHandler({ final: 'c' }, …)` observer (`typings/xterm.d.ts:1742`) only sets a `risPending` flag and returns `false`. Phase 2: the mirror re-baselines in the `terminal.write(chunk, callback)` completion callback of the write that carried the sequence (`node_modules/xterm/src/headless/public/Terminal.ts:173` — the callback fires after the chunk has fully parsed, i.e. after the reset): bump the epoch and rebuild trim/fingerprint state against the now-reset buffer. `risPending` acts as a barrier — coalescer ticks and `pty-rows` serialization are deferred until the re-baseline completes, so reset-buffer contents can never be served under the old epoch. Line indices and watermarks are meaningful only within an epoch. The phone adopts the current epoch at subscribe time, so cross-run monotonicity is not required. Epoch state lives on the **catalog entry**, not the mirror, so a rebind (§1.3) continues the same monotonic sequence — a replacement mirror can never re-issue an epoch the old mirror already served.
