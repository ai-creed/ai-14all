...
- **Confirm/reject SAS clear (both outcomes).** The vendor `ReferenceHost.confirmPairing` never clears `lastSas` — not on reject **and not on accept** (`node_modules/@xavier/xbp/src/reference/host.ts:153-166`; accept sets `peer = pendingPeer` and clears `pendingPeer` only). Without an explicit clear, an accepted confirm would leave `getStatus()` reporting `paired: true` with `pairing: "awaiting-sas"` — a contradiction in the authoritative state machine. Therefore `confirmPairing(ok)` performs the fresh-pairing-host swap and clears `pendingOffer` on **both** outcomes:
	- `ok === false`: swap after the vendor host records its own `rejected` audit entry. Clears the stuck `lastSas` (§2 problem 5) and kills the dead offer token.
	- `ok === true`: swap **after** reading `activePeer()`, attaching the peer, and persisting the device (the swap must not run before the peer's keys are read). Clears `lastSas` so post-pairing status is `paired: true, sas: null, pairing: "idle"`, and kills the spent offer token.
	No extra audit entries beyond the vendor's own in either path.
...
- **Accepted confirm:** peer attached and device persisted first, then host swap; status afterwards is `paired: true, sas: null, pairing: "idle"` — never `paired` with a lingering `awaiting-sas` phase.
...
- **Host service (vitest):** pending-offer lifecycle (set on `startPairing`, cleared on confirm/reject/cancel/forget/stop); lazy expiry + timer-driven status emit (fake timers + injected `now`); `pairing` derivation for all three phases; **reject-clears-sas regression** (fails against today's code); **accepted-confirm-clears-sas regression** — drive a full pairing (`startPairing` → pair-request → `confirmPairing(true)`) and assert the post-confirm status is exactly `paired: true, sas: null, pairing: "idle"` with `offer: null` (fails against an implementation that clears `pendingOffer` but skips the host swap on accept); `cancelPairing` → stale Confirm fail-closed + single `pairing-cancelled` audit entry; `lastError` set on throw and cleared on next success.
