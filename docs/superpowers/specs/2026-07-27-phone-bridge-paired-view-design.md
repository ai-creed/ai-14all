...
- **D5a is a state latch, not a derived prop.** `relayBaseUrl` arrives asynchronously (`settings.read()` in an effect), so the seed cannot be a `useState` initializer; it needs a ref latch that fires on the first resolved load only:
  ...
  **Anti-pattern — `open={relayBaseUrl !== ""}`.** React writes the `open` property on every render, and this panel re-renders on every `onStatusChanged` (line 53), which fires on each relay transition `off → retrying → registered`. A derived prop therefore reopens a disclosure the user just closed, at a moment they are not interacting with it. It satisfies both initial-state assertions, which is why §8 requires an explicit re-render regression.
...
- **`PhoneBridgePanel.test.tsx`** — paired view renders four capability rows; denied rows render no `role="switch"`; toggling "Type into terminals" calls `update` with `ptyInputEnabled` **only**, and "Send notifications to this phone" with `pushWakeEnabled` **only** (assert the patch key, so a crossed wire fails here too). Relay: summary reports `status.relay`, is collapsed when `relayBaseUrl` is empty and open when it is set (D5a). Existing switch locators updated per D8.
  - **D5a re-render regression (required).** Mount with a persisted `relayBaseUrl` (disclosure opens), close the `<details>`, then push an **unrelated** status change through the `onStatusChanged` callback — e.g. `relay: "off" → "retrying"`, or a `pairedAt` bump — and assert the disclosure is **still closed**. The two initial-state assertions above both pass against the `open={relayBaseUrl !== ""}` anti-pattern; only this one fails against it. Drive the re-render through the status callback rather than a manual re-render, so the test exercises the path that actually fires in production.
  - The mirror case: mount with an empty `relayBaseUrl` (collapsed), open the `<details>`, push a status change, assert it stays **open**. Catches a latch that re-seeds on every load rather than once.
