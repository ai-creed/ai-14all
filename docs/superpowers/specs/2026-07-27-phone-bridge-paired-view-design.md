...
- **`css-refactor.visual.spec.ts` (four-theme guard, required by §6/§10).** Add a phone-bridge block to `src/app/UiGallery.tsx` under `data-testid="gallery-phone-bridge"`, then add an element-scoped palette loop over `dark | light | warm | tui`. This follows the existing `sidebar-${palette}.png` pattern in the same file (line ~210) rather than inventing infrastructure.
  ...
  **Captures per palette:**
  ...
  2. **Hover is asserted, not captured** (revised 2026-07-27 after implementation — see below).

  **Why hover has no pixel baseline.** An earlier draft of this section required `phone-bridge-unpair-hover-${palette}.png`, taken after `await unpair.hover()`. That capture was implemented and **failed deterministically on an unmodified tree — 4/4 palettes, 8/8 runs.** Instrumenting it showed why: the element-screenshot operation itself clears `:hover`. `el.matches(":hover")` reads `true` immediately after `.hover()` and `false` immediately after every `toHaveScreenshot`, with `window.scrollY` unchanged; `toHaveScreenshot`'s stability loop captures until two consecutive frames match, so once hover drops during the first capture the frame compared is the resting one.

  The converse is worse than the visible failure: the same race can record the **baseline** un-hovered, in which case the hover guard silently duplicates the rest screenshot and can never fail on a deleted `:hover` rule. A guard that cannot fail is worse than no guard, because it reads as coverage.

  Hover therefore uses the **same two-property computed-style assertion as focus** (route 2 below) — read `border-color` and `color` at rest, hover, read again, assert both switched to the resolved `--danger` and both differ from resting. This is the section's own stated fallback applied to the adjacent case: route 2 is already named "the required floor" when the pixel route proves unreliable in this harness, and hover is exactly that situation. Verified non-vacuous by mutation: renaming the `.phone-bridge__btn--quiet-danger:hover/:focus-visible` selector fails the hover assertion with `Received` = `--muted-foreground` (the value the *base* hover rule sets), proving the read observes a genuinely hovered element rather than the resting one.
