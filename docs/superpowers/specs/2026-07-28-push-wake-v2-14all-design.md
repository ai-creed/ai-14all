...
| --- | --- | --- |
| non-member (or first sight, against an **established** seen-state) → `waiting` or `failed` | ✅ | Entering the trigger set; a null-load baseline pass records without firing (see the null-load rule below) |
...
export type PushWakeWatcherStateV2 = {
	version: 2;
	// Each namespace is independently nullable: null = "this detector has never
	// completed a successful baseline pass" — durably distinct from an
	// established-but-empty namespace, which fires first-sight. The two
	// detectors never share fields.
	whisper: PushWakeSeenState | null;   // existing shape, untouched: workflows, pingedWorkflows, pingedChains
	attention: {
...
`load()` migrates: a legacy v1 file (today's bare `PushWakeSeenState`) becomes `{ version: 2, whisper: <legacy>, attention: { sessions: {} }, lastPingAt: null }` — note the migrated attention namespace is *established and empty*, **not** `null`: nothing attention-related was ever pinged before v2, so first-sight firing there is a first ping, not a duplicate. An unreadable/invalid file loads as `null` (both namespaces conceptually `null`); a valid v2 file may carry `null` for either namespace independently.

**Null-baseline rule (per detector, durable).** A `null` namespace means *nothing is known about what was already pinged for that detector*, and firing against the unknown risks the forbidden duplicate. The rules:

- A detector's namespace starts `null` (fresh install, corrupt file, or store-level `null` load) and stays `null` until that detector's **first successful pass**, which records the current snapshot without firing and thereby establishes the namespace — possibly as *established-empty*, which is a different durable value than `null`. For attention, a *resolved* report counts as a successful pass even when `sessions: []` (empty is authoritative, §2 above); a rejected poll does not. For whisper, a blank read (`states.length === 0`) is ambiguous and does not count; whisper's Arc B in-memory baseline discipline is unchanged, now serialized faithfully.
- The transition table's first-sight row fires only against an **established** (non-`null`) namespace — a session appearing after that detector's baseline pass.
- **Durability through partial initialization:** when one detector is still `null` and the *other* detector's event triggers a composite `save()`, the un-baselined namespace serializes as `null` — never as an empty object. Without this, `attention: {}` written during a whisper-only save would be indistinguishable from the established-empty migration state, and an unchanged `waiting` session would fire after recovery or restart — exactly the forbidden duplicate.

A lost `lastPingAt` claims no coalesce window, which merely permits one earlier ping for a *fresh* post-restart transition — that is a new ping, not a re-ping, and allowed.

**Atomic write mechanism.** Atomicity is a required mechanism, not an aspiration: today's store overwrites the target in place (`writeFileSync` at `push-wake-state-store.ts:40`), which a crash mid-write can truncate — and truncated state means a `null` load and lost deduplication. `save()` must write a temp file in the **same directory** (`push-wake-state.json.tmp`) and atomically rename it over the target, returning `true` only after the rename succeeds. A failure at any point before the rename completes leaves the prior valid file intact on disk.

Arc B's ordering rule is inherited verbatim: **persist before send** — a crash between the two loses a ping (the phone's pull covers it); the reverse order could re-ping a settled session, which is forbidden.
...
3. **Global coalesce** — if any ping attempt (either detector) occurred within the last **60s**, skip and audit `coalesced`. Exact semantics of the timestamp (the `lastPingAt` field of the composite state, §2):
   ...
   - It advances **only** on ticks that actually reach gate 5 — suppressed and coalesced ticks must not touch it, or a flapping session could extend the coalesce window indefinitely without a single ping going out. The watcher's **prechecked `no-token`** skip never reaches the sender either, so it must not advance `lastPingAt` — otherwise a token registered moments later would face a false 60s window; only the sender-raced `no-token` (a real attempt) advances it.
...
**Per-detector blank scoping:** the current whole-tick early return on an empty whisper read (`push-wake-watcher.ts:40`) must become whisper-scoped, or attention detection would be blocked whenever whisper has no states — the common dogfood case this feature exists for. Each source keeps its own blank discipline: whisper `getStates().length === 0` → skip the whisper pass only (`mem-2026-07-03` unchanged for that source); attention provider rejection → skip the attention pass only (§2). Either detector proceeds alone. **Namespace preservation:** when a skipped pass coincides with the other detector's event, the composite `save()` (gate 4) still runs — the skipped detector's namespace must ride through it **verbatim from the last known state**, never reset or defaulted. Persisting an empty namespace for the skipped side would re-fire every unchanged session the moment its source recovers, which is exactly the once-per-transition violation this table forbids. Verbatim includes `null`: a detector that has never completed a successful baseline pass serializes as `null` through the other detector's saves (§2 durability rule), so neither an in-session recovery nor a restart can convert "not yet baselined" into "established empty" and fire on an unchanged session.
...
- **State store:** the composite `PushWakeWatcherStateV2` round-trips, **including `null` namespaces** — a valid file with `attention: null` (or `whisper: null`) loads exactly so, durably distinct from migration's established-empty `{ sessions: {} }`; a legacy v1 file migrates to `{ version: 2, whisper: <legacy>, attention: { sessions: {} }, lastPingAt: null }`; an unreadable/invalid file loads as `null`; `save()` returns `true` on a durable write and `false` on a write failure (injected fs error), with the warn log intact; **atomic replacement** — `save()` demonstrably writes the same-directory temp file then renames (observable via the injected fs seam), and an injected failure *before* the rename completes (temp-write error or rename error) leaves the prior valid state on disk and loadable, not truncated and not `null`.
- **Attention detector (pure):** the full transition table above, including first-sight, unknown attention values (ignored), the **last-session-disappears/reappears** regression (valid empty report prunes; reappearance fires as first sight), and rejection-in-between (no prune, no fire); the **null-load baseline rule** — a `null` seen-state pass over sessions already at `waiting`/`failed` records them and fires nothing, while a migrated *empty-but-established* `attention.sessions` fires first-sight for the same snapshot.
- **Watcher (integration):** gate order (enabled → suppressed-connected → coalesced → persist → send); **two-tick assertions for every consuming skip path** — for each of `suppressed-connected`, `coalesced`, and `no-token`: tick 1 with a fresh transition emits exactly one audit entry and persists the advanced seen-state, and tick 2 with unchanged inputs emits zero events, zero audit entries, and no send (once-per-transition proven through the skip, not just the send path); **persist-failed fail-quiet** — when the composite `save()` returns `false` the sender is *not called* (spy assertion, not call-order alone) and in-memory seen-state does not advance, so the next tick retries; a continuing failure streak emits no second `persist-failed` entry, and a recovered save lets the same transition proceed to its real outcome; coalesce timestamp advances pre-send and outcome-independently, does **not** advance on suppressed, coalesced, **or prechecked `no-token`** ticks (the no-token case additionally asserts its seen-state *was* consumed while `lastPingAt` stayed put — otherwise an implementation could mint a false 60s window), and survives a simulated restart without double-pinging; **missing/corrupt-state restart** — with sessions already at `waiting`, an absent or corrupt state file at watcher start yields a baseline pass that sends nothing and audits nothing, and a *subsequent* fresh transition fires; **namespace preservation, both directions** — (a) attention poll rejects while a whisper event fires and is saved: the persisted `attention` namespace equals the pre-tick value, and a recovery tick with the same unchanged `waiting` session emits no event, no audit, no send; (b) whisper reads empty while an attention event fires and is saved: the persisted `whisper` namespace equals the pre-tick value, and a recovered unchanged whisper read emits nothing; **partial null-load initialization, both directions** — (a) from `load() === null`, the attention poll rejects while a whisper event fires and is saved: the file's `attention` field is asserted `null` (not `{}`), and then *both* an in-session attention recovery and a fresh restart yield a first successful attention pass that records the same unchanged `waiting` session without firing, while a later fresh transition does fire; (b) the converse: whisper reads blank at null initialization while an attention event fires and is saved, the persisted `whisper` field is `null`, and whisper's eventual first successful pass baselines its snapshot without firing; per-detector blank scoping (whisper empty read doesn't block the attention pass and vice versa); one-ping-per-tick with both detectors firing; the audit table in §4 — every ✅ row produces its exact entry, every ❌ row produces none.
