...
`load()` migrates: a legacy v1 file (today's bare `PushWakeSeenState`) becomes `{ version: 2, whisper: <legacy>, attention: { sessions: {} }, lastPingAt: null }`; an unreadable/invalid file loads as `null`.

**Null-load baseline rule.** A `null` seen-state means *nothing is known about what was already pinged*, and firing against the unknown risks the forbidden duplicate. So each detector's first pass after a `null` load **records the current snapshot without firing**: whisper keeps its Arc B baseline discipline unchanged, and the attention detector registers every session currently in the trigger set as already-seen. The transition table's first-sight row fires only against an *established* (non-null) seen-state — a session appearing after the baseline pass. Note the asymmetry with the v1→v2 migration: a migrated file has an *empty but established* `attention.sessions`, so attention first-sight fires there — correct, since nothing attention-related was ever pinged before v2. A lost `lastPingAt` claims no coalesce window, which merely permits one earlier ping for a *fresh* post-restart transition — that is a new ping, not a re-ping, and allowed.
