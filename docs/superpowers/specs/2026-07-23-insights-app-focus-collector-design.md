...
- An **engaged sub-span** exists only inside a focused span and measures **input-active time**, so it is only ever counted up to the last observed input — never up to poll time. On every idle poll while focused, let `lastInput = now − idleSeconds*1000` (the moment of the most recent input):
  - **Below threshold** (`idleSeconds < 60 s`, still engaged): if `lastInput > engagedBoundary`, emit the engaged segment `[engagedBoundary, lastInput]` (reason `poll`) and advance `engagedBoundary = lastInput`; if engaged was closed, it (re)opens at `lastInput`. The segment ends at `lastInput`, **not** at `now`, so the sub-poll idle tail after the last input is never counted.
  ...
  This makes the *never-inflates* invariant hold by construction: every engaged segment's end is a real last-input timestamp, regardless of poll timing. Close reasons: `poll` | `idle` | `blur` | `suspend` | `quit`.
...
- **`focus-core` state machine** (no Electron): a focus→(poll)→(poll)→blur sequence yields **contiguous** `app.focused` segments (reason `poll`) that sum to the total focused duration, the last closed on `blur`; a **nonzero-idle below-threshold poll ends the engaged segment at `pollNow − idleSeconds` (the last input), not poll time** — e.g. a poll at t=15 with idle 5 s ends the segment at t=10, and a later threshold-crossing poll at t=75 (idle 65 s) closes with **no engaged duration after t=10** (reason `idle`), proving no segment extends past the true last-input timestamp; input resuming opens a new engaged span; engaged never exists outside a focused span; **`suspend`→`resume` closes the uptime interval and opens a new one**; **`stop` (disable) drops open focused/engaged spans (no row) but finalizes the `app.uptime` interval** (reason `disabled`); `quit`/`flush` close open spans; a backward-time span clamps to zero.
