...
- **Delete-all → discard the outbox.** The existing `closeStore` → `storeClosed` handshake runs (an intentional stop) and the `insights/` directory is removed; crucially, delete-all also **clears the outbox buffer, discarding all unacked events**, so nothing captured before the wipe can replay into a re-enabled store. A re-enable then brings up a fresh v1 store with an empty buffer, and only spans captured *after* re-enable are delivered. (Contrast a plain worker crash, where unacked events *do* replay — delete-all is a deliberate data wipe, so discarding its buffer is the correct divergence.)
...
electron/main/services/
  insights-host.ts            -- compose outbox; produce(); worker `exit` listener → clear state + consent-gated re-fork; replay-after-config on spawn; disable drains the finalizing uptime (closeStore→storeClosed) before teardown; delete-all discards the outbox; queryAppTime(); start/stop collector with setEnabled (edit)
