...
test.describe.serial("Cumulative flow — Phase 7", () => {
	...
	test("reflects branch rename in the sidebar after Refresh review", async () => {
		...
		await expect(worktreeNav().getByText("feature-b-renamed")).toBeVisible({
			timeout: 15_000,
		});
	});
	...
	test("picks up an externally added worktree after Refresh review", async () => {
		...
		execSync(
			`git -C "${testRepo.repoPath}" branch outside-added origin/master`,
			{
				stdio: "ignore",
			},
		);
		...
		await page.getByRole("button", { name: "Refresh review" }).click();
		await expect(
			worktreeNav().getByRole("button", { name: "outside-added", exact: true }),
		).toBeVisible({
			timeout: 15_000,
		});
	});
	...
	test("removes feature-a via the Remove Worktree dialog and deletes its branch", async () => {
		...
		expect(
			execSync(`git -C "${testRepo.repoPath}" branch --list feature-a`)
				.toString()
				.trim(),
		).toBe("");
	});
	...
	test("drops an externally removed worktree from the sidebar after Refresh review", async () => {
		...
		const externalPath = join(testRepo.repoPath, ".worktrees", "outside-added");
		execSync(
			`git -C "${testRepo.repoPath}" worktree remove "${externalPath}" --force`,
			{
				stdio: "ignore",
			},
		);
		execSync(`git -C "${testRepo.repoPath}" branch -D outside-added`, {
			stdio: "ignore",
		});
		await page.getByRole("button", { name: "Refresh review" }).click();
		await expect(
