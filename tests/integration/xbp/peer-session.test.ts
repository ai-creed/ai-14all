...
import { sessionReportCapability, SESSION_CHANGED_TOPIC } from "@ai-creed/command-contract";
...
async function setupPairedSession() {
  ...
  const audit = new XbpAuditSink({ dir: mkdtempSync(join(tmpdir(), "xbp-ps-neg-")) });
  ...
  const client = new Peer({ backend, identity: clientIdentity, transport: tappedClientT });
  const hostNode = client.addPeer(hostIdentity.sign.publicKey, hostIdentity.box.publicKey, []);
  client.start();
  return { backend, audit, hostIdentity, clientIdentity, hostNode, clientT, sentFrames, session, client };
}

const rejectedReasons = (audit: XbpAuditSink) =>
  audit.entries().filter((e) => e.outcome === "rejected").map((e) => e.reason);
...
describe("XbpPeerSession (in-memory)", () => {
  it("serves a schema-valid session-report and delivers a coalesced session-changed event", async () => {
    const backend = await createNodeSodiumBackend();
    const [hostT, clientT] = createInMemoryPair();
    const audit = new XbpAuditSink({ dir: mkdtempSync(join(tmpdir(), "xbp-ps-")) });
    const hostIdentity = generateIdentity(backend);
    const clientIdentity = generateIdentity(backend);
    ...
    const client = new Peer({ backend, identity: clientIdentity, transport: clientT });
    const hostNode = client.addPeer(hostIdentity.sign.publicKey, hostIdentity.box.publicKey, []);
    const events: string[] = [];
  ...
  it("rejects an unauthorized session-report call (peer lacks control:read) and audits permission-denied", async () => {
    const backend = await createNodeSodiumBackend();
    const [hostT, clientT] = createInMemoryPair();
    const audit = new XbpAuditSink({ dir: mkdtempSync(join(tmpdir(), "xbp-ps-unauth-")) });
    const hostIdentity = generateIdentity(backend);
    const clientIdentity = generateIdentity(backend);
    const session = new XbpPeerSession({
      backend,
    ...
    session.attach(clientIdentity.sign.publicKey, clientIdentity.box.publicKey, []); // NO permission granted

    const client = new Peer({ backend, identity: clientIdentity, transport: clientT });
    const hostNode = client.addPeer(hostIdentity.sign.publicKey, hostIdentity.box.publicKey, []);
    client.start();

    await expect(client.call(hostNode, sessionReportCapability, {})).rejects.toBeTruthy();
    await new Promise((r) => setTimeout(r, 10));
    expect(
      audit.entries().some((e) => e.outcome === "rejected" && e.reason === "permission-denied"),
    ).toBe(true);
    session.stop();
  });
  ...
  it("returns an error (never crashes the service) and audits handler-error when the handler throws", async () => {
    const backend = await createNodeSodiumBackend();
    const [hostT, clientT] = createInMemoryPair();
    const audit = new XbpAuditSink({ dir: mkdtempSync(join(tmpdir(), "xbp-ps-throw-")) });
    const hostIdentity = generateIdentity(backend);
    const clientIdentity = generateIdentity(backend);
    const session = new XbpPeerSession({
      backend,
    ...
    await new Promise((r) => setTimeout(r, 10));
    expect(
      audit.entries().some((e) => e.outcome === "rejected" && e.reason === "handler-error"),
    ).toBe(true);
    session.stop();
  });

  it("rejects + audits a replayed request frame on the live Peer path", async () => {
    const { audit, hostNode, clientT, sentFrames, client, session } = await setupPairedSession();
  ...
  it("rejects + audits a tampered (corrupted-seal) frame on the live Peer path", async () => {
    ...
    if (!frame || frame.t !== "addressed") throw new Error("expected an addressed frame");
  ...
  it("rejects + audits a forged-signature frame on the live Peer path", async () => {
    const { backend, audit, hostIdentity, hostNode, clientT, client, session } = await setupPairedSession();
    ...
    const forgedInner = sealAndSign(backend, utf8("forged"), attacker.sign.privateKey, hostIdentity.box.publicKey);
