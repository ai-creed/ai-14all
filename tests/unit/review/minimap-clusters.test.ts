...
import {
	clusterDots,
	type Dot,
} from "../../../src/features/review/logic/minimap-clusters";

const d = (id: string, position: number): Dot => ({
	id,
	position,
	status: "open",
});
...
describe("clusterDots", () => {
	...
	it("merges dots within the threshold", () => {
		const out = clusterDots([d("a", 0.5), d("b", 0.51)], 0.02);
	...
	it("collapses dense regions: 200 evenly-spaced dots with threshold 0.02 produce fewer than 200 clusters", () => {
		const dots: Dot[] = Array.from({ length: 200 }, (_, i) =>
			d(`d${i}`, i / 200),
		);
