...
const base: Status = { enabled: true, listening: true, addr: "10.0.0.5", port: 51820, paired: false, sas: null };
  ...
  (window as unknown as { ai14all: unknown }).ai14all = {
    phoneBridge: {
      ...
      startPairing: vi.fn().mockResolvedValue({
        offer: JSON.stringify({ token: "t", signPubHex: "aa", boxPubHex: "bb", connect: { url: "ws://10.0.0.5:51820" }, expiresAt: 9 }),
      }),
...
describe("PhoneBridgePanel", () => {
  it("shows the listening address once status resolves", async () => {
    ...
    await waitFor(() => expect(screen.getByText(/10\.0\.0\.5:51820/)).toBeInTheDocument());
  });

  it("renders the pairing QR after clicking Pair a phone", async () => {
    mountBridge(base);
    render(<PhoneBridgePanel />);
    await userEvent.click(await screen.findByRole("button", { name: /pair a phone/i }));
    await waitFor(() => expect(window.ai14all.phoneBridge.startPairing).toHaveBeenCalled());
  ...
  it("lists the paired device once paired", async () => {
    ...
    render(<PhoneBridgePanel />);
    await waitFor(() => expect(screen.getByText(/paired/i)).toBeInTheDocument());
  });
});
