...
function sessionWith(state: "active" | "waiting", summary: string): WorktreeSession {
	return {
		id: "s",
		worktreeId: "wt1",
		title: "t",
...
describe("createSamanthaSliceBuilder", () => {
	...
	it("appends a transition to the ring when the resolved attention changes", () => {
		...
		t = 200;
		const slice = builder.build(
			[{ worktreeId: "wt1", session: sessionWith("waiting", "needs you"), processSessionsById: {} }],
			"wt1",
			"ready",
		);
		expect(slice.worktrees[0].recent).toEqual([
			{ at: 200, from: "active", to: "waiting", summary: "needs you", source: "mcp" },
		]);
	});
	...
	it("does not append when attention is unchanged", () => {
		...
		builder.build(
			[{ worktreeId: "wt1", session: sessionWith("active", "a"), processSessionsById: {} }],
			"wt1",
			"ready",
		);
		const slice = builder.build(
	...
	it("caps the ring at ringSize", () => {
		...
		const states: ("active" | "waiting")[] = ["active", "waiting", "active", "waiting"];
		let slice = builder.build(
			[{ worktreeId: "wt1", session: sessionWith("active", "x"), processSessionsById: {} }],
			"wt1",
			"ready",
		);
		for (const s of states) {
			t += 10;
			slice = builder.build(
				[{ worktreeId: "wt1", session: sessionWith(s, "x"), processSessionsById: {} }],
				"wt1",
				"ready",
			);
		}
