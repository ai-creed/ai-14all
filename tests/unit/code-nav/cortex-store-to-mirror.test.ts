...
describe("ingestCortexStore", () => {
	...
	it("derives bare_name (incl. nested) and carries function ranges", () => {
		...
		const cli = db
			.prepare(
				"SELECT bare_name, col, end_line FROM functions WHERE qualified_name='Cli::parse'",
			)
			.get() as {
			bare_name: string;
			col: number | null;
		...
		const pc = db
			.prepare(
				"SELECT bare_name, col, end_line, end_col FROM functions WHERE qualified_name='parseConfig'",
			)
			.get() as {
			bare_name: string;
			col: number;
	...
	it("skips when fingerprint and schema_version match; re-ingests when fingerprint differs", () => {
		...
		expect(ingestCortexStore(cortexDb, mirror)).toMatchObject({
			skipped: true,
		});
		makeCortexFixtureDb(cortexDb, {
			meta: { fingerprint: "fp2" },
			functions: [{ qualified_name: "x", file: "x.ts", line: 1 }],
		});
		expect(ingestCortexStore(cortexDb, mirror)).toMatchObject({
			skipped: false,
		});
	...
	it("returns no-store when the cortex .db is missing", () => {
		const result = ingestCortexStore(
			join(dir, "missing.db"),
			join(dir, "m.sqlite"),
		);
	...
	it("rejects schemaVersion 3.0 and 4.0; accepts 3.1", () => {
		...
		makeCortexFixtureDb(ok, {
			meta: { schemaVersion: "3.1" },
			functions: [{ qualified_name: "a", file: "a.ts", line: 1 }],
		});
		expect(ingestCortexStore(ok, join(dir, "ok.sqlite"))).toMatchObject({
			skipped: false,
		});
	});
});
