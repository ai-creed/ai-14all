...
describe("CortexStoreReader", () => {
	...
	it("readMeta parses meta keys and defaults dirtyAtIndex", () => {
		...
		makeCortexFixtureDb(p, { meta: { schemaVersion: "3.1", fingerprint: "fpX" } });
	...
	it("readGraph returns functions/calls/imports/files with cortex column names", () => {
		...
		makeCortexFixtureDb(p, {
			functions: [
				{ qualified_name: "foo", file: "a.ts", line: 1, col: 1, end_line: 3, end_col: 2 },
			],
			calls: [
				{ from_key: "a.ts::foo", to_key: "::Set", kind: "new", site_line: 2, site_col: 4 },
			],
			imports: [{ from_path: "a.ts", to_path: "b.ts" }],
			files: [{ path: "a.ts", kind: "file" }],
		});
		const g = new CortexStoreReader(p).readGraph();
		expect(g.functions[0]).toMatchObject({ qualified_name: "foo", file: "a.ts", end_line: 3 });
		expect(g.calls[0]).toMatchObject({ from_key: "a.ts::foo", to_key: "::Set", site_line: 2 });
		expect(g.imports).toEqual([{ from_path: "a.ts", to_path: "b.ts" }]);
		expect(g.files).toEqual([{ path: "a.ts", kind: "file", content_hash: null }]);
	});
});
