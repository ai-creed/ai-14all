...
describe("XbpIdentityStore", () => {
  ...
  it("generates, persists, and reloads a stable identity (stable NodeId)", async () => {
    ...
    const a = new XbpIdentityStore({ dir, backend, secureStorage: fakeSecureStorage() }).load();
    const b = new XbpIdentityStore({ dir, backend, secureStorage: fakeSecureStorage() }).load();
    expect(b.nodeId).toBe(a.nodeId);
    expect([...b.identity.sign.publicKey]).toEqual([...a.identity.sign.publicKey]);
  });

  it("fails closed when secure storage is unavailable — never writes plaintext", async () => {
    const backend = await createNodeSodiumBackend();
    const store = new XbpIdentityStore({ dir, backend, secureStorage: fakeSecureStorage(false) });
