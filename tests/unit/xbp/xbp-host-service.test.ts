...
import { createNodeSodiumBackend, generateIdentity, toHex } from "@xavier/xbp/node";
...
describe("XbpHostService", () => {
  ...
  it("fails closed when secure storage is unavailable", async () => {
    ...
    await expect(svc.start()).rejects.toBeInstanceOf(XbpSecureStorageUnavailableError);
