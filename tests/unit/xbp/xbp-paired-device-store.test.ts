...
import { XbpSecureStorageUnavailableError, type SecureStorage } from "../../../services/xbp/xbp-identity-store";
...
describe("XbpPairedDeviceStore", () => {
  ...
  it("returns null before anything is paired", () => {
    expect(new XbpPairedDeviceStore({ dir, secureStorage: fakeSecureStorage() }).load()).toBeNull();
  });
  ...
  it("saves and reloads the paired device across instances (survives restart)", () => {
    ...
    new XbpPairedDeviceStore({ dir, secureStorage: fakeSecureStorage() }).save(device);
    const reloaded = new XbpPairedDeviceStore({ dir, secureStorage: fakeSecureStorage() }).load();
    expect(reloaded).toEqual(device);
  });

  it("clear() forgets the device", () => {
    const store = new XbpPairedDeviceStore({ dir, secureStorage: fakeSecureStorage() });
  ...
  it("fails closed on save when secure storage is unavailable — never writes plaintext", () => {
    const store = new XbpPairedDeviceStore({ dir, secureStorage: fakeSecureStorage(false) });
    expect(() => store.save({ signPubHex: "aa", boxPubHex: "bb", pairedAt: 1 })).toThrow(XbpSecureStorageUnavailableError);
  });
});
