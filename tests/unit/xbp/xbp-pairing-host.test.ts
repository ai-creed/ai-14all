...
async function makeHost(opts: { now?: () => number; ttl?: number } = {}) {
  ...
  const audit = new XbpAuditSink({ dir: mkdtempSync(join(tmpdir(), "xbp-ph-")) });
...
describe("XbpPairingHost", () => {
  it("creates a parseable offer carrying the LAN url and host keys", async () => {
    ...
    expect(parsed?.connect.url).toBe("ws://10.0.0.5:51820");
  ...
  it("rejects a pair-request presented after the token TTL — no pairing, audited 'rejected'", async () => {
    let clock = 1_000_000;
    const { backend, audit, host } = await makeHost({ now: () => clock, ttl: 1_000 });
    const offer = host.createOffer({ url: "ws://10.0.0.5:51820" });
    const client = new ReferenceClient({ backend, identity: generateIdentity(backend) });
