...
describe("XbpAuditSink", () => {
  it("appends one JSONL line per decision, with a timestamp, readable via entries()", () => {
    ...
    sink.append({ cap: "xavier.control.session-report", risk: "low", outcome: "accepted" });
    sink.append({ cap: null, risk: null, outcome: "rejected", reason: "bad-signature" });
  ...
  it("never throws when the directory is unwritable", () => {
    ...
    expect(() => sink.append({ cap: null, risk: null, outcome: "rejected" })).not.toThrow();
