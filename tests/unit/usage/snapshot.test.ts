...
describe("buildSnapshot coherence", () => {
	it("within each scope, totalTokens == sum(byProvider) == sum(rows), and cost is from the same window", () => {
		...
		ingestEvent(
			ledger,
			session,
			ev({ timestampMs: now - HOUR, billable: 7, raw: 70 }),
			now - 2 * HOUR,
		); // post-launch
		ingestEvent(
			ledger,
			...
			now - 2 * HOUR,
		); // pre-launch (before launch = now-2h? it's now-3h => pre-launch)
		const snap = buildSnapshot({
			...
			chipRange: "week",
			providersWithData: new Set(["codex", "claude"]),
			codexLimits: null,
		});
		for (const scope of [
			"session",
	...
	it("emits both chart series and all four scope keys", () => {
		...
		expect(Object.keys(snap.scopes).sort()).toEqual([
			"all-time",
			"month",
			"session",
			"week",
	...
	it("groups a deleted worktree's all-time tokens under its workspace", () => {
		...
		ingestEvent(
			...
			session,
			ev({
				cwd: "/Users/me/Dev/app/.worktrees/gone",
		...
		const snap = buildSnapshot({
			...
			chipRange: "week",
			providersWithData: new Set(["codex"]),
			codexLimits: null,
		});
		const row = snap.scopes["all-time"].rows.find(
			(r) => r.workspaceId === "ws-app",
		);
	...
	it("does NOT attribute a sibling repo to another repo's workspace (only shares a parent dir)", () => {
		...
		expect(
			snap.scopes["all-time"].rows.find((r) => r.workspaceId === "ws-app"),
		).toBeUndefined();
		const untracked = snap.scopes["all-time"].rows.find(
