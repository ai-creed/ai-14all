...
describe("rateFor (blended per-provider)", () => {
	it("prices every known provider with its median rate, ignoring model", () => {
		...
		expect(claude).toEqual({ inputPerM: 3, outputPerM: 15, cacheReadPerM: 0.3 });
		const codex = rateFor("codex");
		expect(codex).toEqual({ inputPerM: 1.25, outputPerM: 10, cacheReadPerM: 0.125 });
	...
	it("falls back to GLOBAL_AVG for an unknown provider (never null)", () => {
		...
		expect(rateFor("cursor")).toEqual({ inputPerM: 2, outputPerM: 12, cacheReadPerM: 0.2 });
		expect(rateFor("antigravity")).toEqual({ inputPerM: 2, outputPerM: 12, cacheReadPerM: 0.2 });
	});
});
