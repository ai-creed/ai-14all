			...
			{remoteStatus && (
				<div className="flex items-center gap-3 px-3 py-2 border-b border-border text-xs">
					<span className="flex gap-2 text-muted-foreground flex-1">
						<span className="inline-flex items-center" aria-label={`${remoteStatus.ahead} ahead`}><ArrowUp className="h-3 w-3" aria-hidden="true" />{remoteStatus.ahead}</span>
						<span className="inline-flex items-center" aria-label={`${remoteStatus.behind} behind`}><ArrowDown className="h-3 w-3" aria-hidden="true" />{remoteStatus.behind}</span>
					</span>
					<button
						type="button"
