...
function collectMatchAncestors(paths: string[], term: string): { matches: Set<string>; ancestors: Set<string> } {
...
export function flattenTreeToRows(input: FlattenTreeInput): VisibleRow[] {
	...
	const rootExpanded = searching ? true : input.expandedPaths.has(WORKTREE_TREE_ROOT_PATH);
	rows.push({ kind: "dir", path: WORKTREE_TREE_ROOT_PATH, name: input.rootLabel, depth: 0, expanded: rootExpanded, hasChildren: input.tree.length > 0 });
	...
	function walk(nodes: FileTreeNode[], depth: number): void {
		for (const node of nodes) {
			if (node.type === "file") {
				...
				rows.push({ kind: "file", path: node.path, name: node.name, depth, gitStatus: input.changedFiles.get(node.path) });
				continue;
			}
			if (searching && !ancestors!.has(node.path)) continue;
			const expanded = searching ? true : input.expandedPaths.has(node.path);
			rows.push({ kind: "dir", path: node.path, name: node.name, depth, expanded, hasChildren: node.children.length > 0 });
