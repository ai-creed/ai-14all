...
function usageOf(info) {
  if (!info) return null;
  return info.last_token_usage ?? info.total_token_usage ?? null;
}
  ...
  for (const record of readJsonl(file)) {
    ...
    if (payload.rate_limits) {
      latestLimit = {
        ...
        primary_window_minutes: payload.rate_limits.primary?.window_minutes ?? null,
        primary_resets_at: fmtReset(payload.rate_limits.primary?.resets_at),
        secondary_used_percent: payload.rate_limits.secondary?.used_percent ?? null,
        secondary_window_minutes: payload.rate_limits.secondary?.window_minutes ?? null,
...
function printTable(title, rows, keyName) {
  ...
  for (const row of rows) {
    for (const h of headers) widths[h] = Math.max(widths[h], String(row[h]).length);
  }

  console.log(headers.map((h) => String(h).padStart(widths[h])).join("  "));
  for (const row of rows) {
    console.log(headers.map((h) => String(row[h]).padStart(widths[h])).join("  "));
  }
}
...
if (asJson) {
  ...
  console.log("-------------------------");
  console.log(latestLimit ? JSON.stringify(latestLimit, null, 2) : "(no rate limit snapshot found)");
}
